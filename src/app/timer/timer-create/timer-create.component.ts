...
export class TimerCreateComponent implements OnInit {
    ...
    public generatedName: string = 'Timer#' + Math.floor(1000 + Math.random() * 9000);
    ...
    public async chooseTime(): Promise<void> {
        ...
        picker.onDidDismiss().then(async data => {
            ...
            if (!canceled) {
                console.log('Selected minute: ' + minCol.options[minCol.selectedIndex].text);
                console.log('Selected second: ' + secCol.options[secCol.selectedIndex].text);
                this.minutes.next(minCol.options[minCol.selectedIndex].value);
                this.seconds.next(secCol.options[secCol.selectedIndex].value);
                this.timerForm.get('minutes').setValue(minCol.options[minCol.selectedIndex].value);
                this.timerForm.get('seconds').setValue(secCol.options[secCol.selectedIndex].value);
            }
        });
    }
}
