...
export class TimerCreateComponent implements OnInit {
    ...
    public async createTimer({ valid, value }: { valid: any; value: any }): Promise<void> {
        if (!valid) {
            return;
        }
    ...
    public async chooseTime(): Promise<void> {
        ...
        for (let i = 0; i < minutesInHour; i++) {
            ...
            minutesOptions.push({ value: parseInt(formattedIndex, 10), text: formattedIndex });
        }
        for (let i = 0; i < secondsInMinute; i++) {
            ...
            secondsOptions.push({ value: parseInt(formattedIndex, 10), text: formattedIndex });
        }
